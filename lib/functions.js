...
/**
 *	add custom event handler to given element object
 * 
...
/**
 *	removes custom event handler to given element object
 * 
 *	@param		HTMLElement		elm
 *	@param		STRING			evType
 *	@param		FUNCTION		fn
...
/**
 * build location query string from parameter object
 * 
 * @returns		STRING
 */
var buildQueryString = function ( oParams ) {
	...
	for (key in oParams) {
		if (typeof oParams[key] != 'function') {
			...
			if ((typeof oParams[key].toString == 'function') && (typeof oParams[key] == 'object')) {
				query += '' + key + '' + '=' + encodeURIComponent( oParams[key].toString() );
			} else if ((typeof oParams[key].toString != 'function') && (typeof oParams[key] == 'object')) {
				var subidx = 0;
				for (subkey in oParams[key]) {
					if (subidx > 1) {
						query += '&';
					}
					if (typeof oParams[key][subkey] == 'function') {
						query += key + '[' + subkey + ']' + '=' + encodeURIComponent( oParams[key][subkey] );
					}
					subidx++;
				}
			} else {
				query += '' + key + '' + '=' + encodeURIComponent( oParams[key] );
			}
			index++;
		}
	}
...
if ( typeof document.getElementsByTagAttr != 'function' ) {
	document.getElementsByAttr = function ( tags, attribute, value ) {
		var aTags = tags;
		var result = [];
 ...
 * 
 * @param		OBJECT	urlParameters (url values)
 * @param		OBJECT	params (url keys)
 * @returns		STRING|BOOLEAN
 */
var buildQueryString2 = function ( urlParameters, params ) {
	var queryString = [];
